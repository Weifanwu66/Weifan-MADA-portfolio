...
# Model `BodyTemp` based on all predictors: ChillsSweats, Vomit, Weakness, ChestPain, and RunnyNose with/ without interactions
...
wf_Temp_interact=workflow(rec_Temp_interact,lm_spec)
rs_Temp_interact=fit_resamples(wf_Temp_interact, Temp_folds,control=ctrl_preds)

t3=augment(rs_Temp_interact)%>%
 metrics(truth = !!sym("BodyTemp"), estimate = .pred)%>%
...
Compare_Temp_table=t1%>%
  bind_cols(t2)%>%
  bind_cols(t3)
Compare_Temp_table
```
From the table we can conclude that the model explains the most (R^2=19%) variations of BodyTemp when it includes all the interested predictors without interactions
...
# Build a model using all interetsed predictors.
```{r}
set.seed(123)
sym_split=initial_split(mod_data,strata = Nausea)
sym_train=training(sym_split)
...
This table indicates that when model is trained based on all predictors with no interactions, it has the highest accuracy.